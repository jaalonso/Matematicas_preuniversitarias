...
+ 161 Propiedades del producto escalar:
  + Conmutativa: $\vec{v} \cdot \vec{w} = \vec{w} \cdot \vec{v}$
  + Distributiva respecto a la suma:
    $\vec{u} \cdot (\vec{v} + \vec{w}) = \vec{u} \cdot \vec{v} + \vec{u} \cdot \vec{w}$
  + Asociativa respecto a escalares: $\vec{v} \cdot (k \vec{w}) = k (\vec{v} \cdot \vec{w})$
  + Si $\alpha$ es el ángulo que forman $\vec{v}$ y $\vec{w}$, entonces
    $\vec{v} \cdot \vec{w} = ||\vec{v}|| \cdot ||\vec{w}|| \cos(\alpha)$
